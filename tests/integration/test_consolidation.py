...
async def test_consolidation_skips_with_insufficient_messages(sqlite_db, consolidation_pipeline):
    ...
    async with sqlite_db() as db:
        await db.execute(
            text(
                "INSERT INTO conversations (id, started_at, message_count) VALUES (:id, :ts, :mc)"
            ),
        ...
        for _i in range(2):
            ...
            await db.execute(
                text(
                    "INSERT INTO messages (id, conversation_id, role, content, created_at) VALUES (:id, :sid, :role, :content, :ts)"
                ),
                {
                    "id": msg_id,
                    "sid": session_id,
    ...
    # Mock memory_api — M2.1.5: the pipeline now reads the transcript through
    # MemoryAPI.get_conversation_messages(), which returns typed Message objects.
    from aether.llm import Message

    class MockMemoryAPI:
        async def get_conversation_messages(self, sid):
            return [Message(role="user", content="hello")] * 2
...
async def test_consolidation_creates_long_term_memories(sqlite_db, consolidation_pipeline):
    ...
    class MockMemoryAPI:
        # M2.1.5: the pipeline reads the transcript through
        # MemoryAPI.get_conversation_messages(), returning typed Messages.
        async def get_conversation_messages(self, sid):
            return [
        ...
        async def remember(
            ...
            memory_type,
            importance,
            metadata={},  # noqa: B006 - mirrors the locked MemoryAPI.remember signature
            session_id=None,
            source="CONVERSATION",
        ):
            return "mem_1"
