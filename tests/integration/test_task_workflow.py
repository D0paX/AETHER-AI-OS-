...
async def test_create_task_via_agent(kernel: AetherKernel):
        ...
        tool_calls=[
            {
                ...
                "arguments": {
                    ...
                    "description": "Fix memory module bug",
                    "priority": "NORMAL",
                },
            }
        ],
    ...
    decision2 = AgentDecision(thought="Task created.", final_answer="I have created the task.")

    mock_complete.side_effect = [
        AsyncMock(
            usage=AsyncMock(total_tokens=10), cost_usd=0.01, content=decision1.model_dump_json()
        ),
        AsyncMock(
            usage=AsyncMock(total_tokens=10), cost_usd=0.01, content=decision2.model_dump_json()
        ),
    ]
...
async def test_complete_task_via_agent(kernel: AetherKernel):
    # Pre-create a task
    task_id = await kernel.task_manager.create_task(
        title="Write unit tests", description="Write them", priority="HIGH"
    )
    ...
    decision2 = AgentDecision(
        thought="Task updated.", final_answer="The unit tests task has been marked as completed."
    )
