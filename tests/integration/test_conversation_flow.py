...
from aether.agents.base import AgentTask, AgentContext
from aether.agents._implementations.conversation import AgentDecision
from aether.memory.models import MemoryRecord, MemoryType, MemorySource, ContextPackage
...
async def test_cross_session_memory():
    ...
    with patch("aether.core.kernel.EventBus") as MockKernelEventBus, \
         patch("aether.llm.router.EventBus") as MockRouterEventBus, \
         patch("aether.core.kernel.LLMRouter._connect_bus", new_callable=AsyncMock), \
         patch("aether.memory.api.QdrantMemoryStore") as MockQdrant:
        MockKernelEventBus.return_value.connect = AsyncMock()
        MockKernelEventBus.return_value.emit = AsyncMock()
        MockKernelEventBus.return_value.disconnect = AsyncMock()
        MockRouterEventBus.return_value.connect = AsyncMock()
        MockRouterEventBus.return_value.emit = AsyncMock()
        MockRouterEventBus.return_value.disconnect = AsyncMock()
    ...
    decision1 = AgentDecision(thought="I should store this", final_answer="I have remembered the secret code 42.")
    kernel1.llm_router.complete.return_value = AsyncMock(
        usage=AsyncMock(total_tokens=10),
        cost_usd=0.01,
        content=decision1.model_dump_json()
    )
    ...
    context1 = AgentContext(
        ...
        memory_context=ContextPackage(memories=[], total_found=0, token_estimate=0, formatted_context="", retrieval_query="", retrieval_duration_ms=0), 
        active_tasks=[]
    )
    
    res1 = await kernel1.agent_runtime.execute("conversation_agent", task1, context1)
    ...
    with patch("aether.core.kernel.EventBus") as MockKernelEventBus, \
         ...
         patch("aether.core.kernel.LLMRouter._connect_bus", new_callable=AsyncMock), \
         patch("aether.memory.api.QdrantMemoryStore") as MockQdrant, \
         patch("aether.memory.api.SQLiteMemoryStore") as MockSQLite:
        MockKernelEventBus.return_value.connect = AsyncMock()
        MockKernelEventBus.return_value.emit = AsyncMock()
        MockKernelEventBus.return_value.disconnect = AsyncMock()
        MockRouterEventBus.return_value.connect = AsyncMock()
        ...
        mock_sqlite_instance = MockSQLite.return_value
        mock_sqlite_instance.init_db = AsyncMock()
        mock_sqlite_instance.get_memory = AsyncMock(return_value=
            MemoryRecord(
                id="1", 
    ...
    kernel2.llm_router.complete.return_value = AsyncMock(
        usage=AsyncMock(total_tokens=10),
        cost_usd=0.01,
        content=decision2.model_dump_json()
    )
    ...
    res2 = await kernel2.agent_runtime.execute("conversation_agent", task2, context2)
