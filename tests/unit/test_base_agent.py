...
class DummyAgent(BaseAgent):
    ...
    async def execute(self, task, context):
        return AgentResult(run_id="r1", success=True, response="ok", actions_taken=[], memories_created=[], tasks_modified=[], llm_tokens_used=0, llm_cost_usd=0.0, duration_ms=0)
...
async def test_base_agent_remember():
    ...
    assert res == "mem1"
    memory_api.remember.assert_called_with(content="content", memory_type=MemoryType.EPISODE, importance=0.5, session_id="s1")
...
async def test_base_agent_invoke_tool():
    ...
    event_bus.emit.assert_called_with("tool.execution.completed", {"tool_name": "dummy_tool", "success": True, "agent_name": "dummy", "session_id": None})
